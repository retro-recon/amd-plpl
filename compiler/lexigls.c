/* LEXIGLS.C  April 23, 1986
   Lexical Analyzer returns tokens from a file */
#include "glsdef.h"
/* - - - - - - - - - - - - - - - */
err_write(channel,msg_str)
FILE *channel;
char msg_str[];
{ int i;
fprintf(channel,"\n==> Error %d: %s",errors,msg_str);
...
if (severity == WARNING || severity == FATAL)
  ++errors;
if (out_file != NULL)  /* write to output file */
  err_write(out_file,msg);
err_write(stderr,msg);
if (severity == FATAL)
  exit(BAD_EXIT);
...
screen_position = 0;
lexi_i = 0;
/* print source line to show location during compilation */
if (debug_flag == YES)  /* may be helpful during debug phase */
  fprintf(stderr,"\n%s",source_line);
} /* end of GET_LINE */
/* - - - - - - - - - - - - - - - */
...
for ( i = 1;
   ...
   token[i] = a_char;
if (i >= 40)
  { err(FATAL,"beyond range of machine .. perhaps a Cray XM-P ?",
...
if (token[0] == '#')
  ...
  err(FATAL,"use '#' for numbers",token_type,VARIABLE,PUNCT_MARK);
} /* end of NUMBER_PARSE */
/* - - - - - - - - - - - - - - - */
comments()
{ int line;
...
while (chr = get_char())
 { if (isdigit(chr) || chr == '#')
   ...
   if (isalnum(chr) || chr == '_')     /* read in an alphanumeric token */
      ...
      token[i] = chr;
    if (i >= MAX_NAME-1)
      { err(FATAL,"name too long, use only 28 characters ...",
